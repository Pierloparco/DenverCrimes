...
public class FXMLController {
    ...
    void doCalcolaPercorso(ActionEvent event) {
    	...
    	if(arco == null) {
    		txtResult.appendText("Seleciona un arco!");
    		return;
    	}
    ...
    void doCreaGrafo(ActionEvent event) {
    	...
    	if(categoria == null) {
    		txtResult.appendText("Seleciona una categoria!");
    		return;
    	}
    	Integer mese = this.boxMese.getValue();
    	if(mese == null) {
    		txtResult.appendText("Seleciona un mese!");
    		return;
    	}
    ...
    public void setModel(Model model) {
    	this.model = model;
    	// dopo aver settato il modello, per riempire il menu a tendina con le categorie di reato(offense_category_id)
    	this.boxCategoria.getItems().addAll(this.model.getCategorie());
    	// per il mese che è un intero tra 1 e 12;
